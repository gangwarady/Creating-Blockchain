...
class Blockchain:
    ...
    def proof_of_work(self, previous_proof):
        ...
        while check_proof is False:
            hash_op = hashlib.sha256(str(new_proof**2 - previous_proof**2).encode()).hexdigest() #non symmetrical
            if hash_op[:4] == '0000':
                check_proof = True
            else:
                new_proof += 1
...
def is_valid():
    ...
    if is_valid:
        ...
        response = {'message': 'uh,oh! The Blockchain is not valid'}
    return jsonify(response), 200
